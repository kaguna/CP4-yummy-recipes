    ...
    axiosInstance
      ...
      .then((response) => {
        this.setState({
          ...
          categoryName: response.data.recipes[0].category_name,
      ...
      .catch((error) => {
        if (error.response) {
          if (error.response.status === 402) {
            this.setState({
              noRecipe: error.response.data.message,
              items: '',
              successMessage: '',
              errorMessage: '',
              recipes: [],
            ...
            this.setState({
              noRecipe: 'No recipe with such letter(s).',
              items: '',
              successMessage: '',
              errorMessage: '',
              recipes: [],
  ...
  render() {
    ...
    return (
      <div id="all_recipes">
        ...
        <Toaster />
        <CreateRecipe
          recipeAfterCreation={this.viewRecipesHandler}
          ...
          <div className="col-sm-14">
            <div className="panel panel">
              <div className="panel-body">
                <div className="row">
                  <div className="col-sm-2">
                    <h5>
                      ...
                      Recipe List{' '}
                      <span className="label label-info">{items}</span>
                    </h5>
                  </div>
                ...
                <div>
                  {successMessage ? (
                    <div className="alert alert-success">
                      {successMessage}
                      <a href="" className="pull-right" data-dismiss="alert">
                        X
                      </a>
                    </div>
                  ) : (
                ...
                <div className="row">
                  {recipes.map(recipe => (
                    <div className="col-sm-3" key={recipe.id + 1}>
                      <div className="panel panel-default">
                        ...
                        <div className="panel-body">
                          <div>
                            Date created: {recipe.date_created.substring(0, 17)}{' '}
                            at {recipe.date_created.substring(17, 25)}
                          </div>
                          <hr />
                          <div>
                            Date Updated: {recipe.date_updated.substring(0, 17)}{' '}
                            at {recipe.date_updated.substring(17, 25)}
                          </div>
                        </div>
                        <div className="panel-footer">
                          <div className="row">
                            <a
                              href=""
                              className="btn btn-default col-sm-3 action_buttons"
                              data-toggle="modal"
                              data-target={`#view_procedure${recipe.id}`}
                            >
                              <span className="glyphicon glyphicon-folder-open" />
                            </a>
                            <a
                              href=""
                              className="btn btn-success col-sm-3 action_buttons"
                              data-toggle="modal"
                              data-target={`#edit_recipe${recipe.id}`}
                            >
                              <span className="glyphicon glyphicon-pencil" />
                            </a>
                            <a
                              href=""
                              className="btn btn-danger col-sm-3 action_buttons"
                              data-toggle="modal"
                              data-target={`#delete_recipe${recipe.id}`}
                            >
